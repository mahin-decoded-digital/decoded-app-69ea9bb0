// ---------------------------------------------------------------------------
// Database Client 1 dual mode (gated by PROD flag)
//
...
// them into `new Date()`, string ops, comparisons 1 strict `tsc` rejects
...
// ---------------------------------------------------------------------------
// Exported client 1 auto-selects based on environment
// ---------------------------------------------------------------------------
...
if (IS_PROD && !MONGODB_URI) {
  console.warn('[db] PROD=true but MONGODB_URI not set 1 falling back to in-memory (data will not persist across restarts)')
} else if (!IS_PROD && MONGODB_URI) {
  console.warn('[db] MONGODB_URI is set but PROD!=true 1 ignoring (in-memory mode). This is expected in WebContainer preview.')
}
